...
# ── numpy 型別清洗 ──
def _sanitize_numpy(d: dict) -> dict:
    """將 dict 中所有 numpy 型別轉為 Python 原生型別"""
    out = {}
    for k, v in d.items():
...
# ── 統計工具 (AI Features 用, 0.0~1.0 百分位) ──
def percentile_rank(series, window=252):
    ...
    current = s.iloc[-1]
    hist = s.iloc[-window:]
    return round(float((hist < current).sum() / len(hist)), 4)


def zscore_last(series, window=252):
    """最新值的 z-score"""
    s = series.dropna()
    if len(s) < 20:
        return None
    ...
    if sigma == 0 or pd.isna(sigma):
        return 0.0
    return round(float((s.iloc[-1] - mu) / sigma), 4)


def slope_n(series, n=5):
    """最近 n 筆的線性回歸斜率"""
...
def max_drawdown(series, window=20):
    """近 window 日最大回撤 (負數)"""
...
def calculate_volume_profile(df, lookback=60, n_bins=50):
    """近 lookback 日成交量分布 → POC"""
...
def detect_short_term_sr(df, lookback=20):
    """近 lookback 日高低點 → 短期支撐壓力"""
    sub = df.tail(lookback)
    if sub.empty:
        return {"st_support": None, "st_resistance": None}
    return {"st_support": round(float(sub["Low"].min()), 2),
            "st_resistance": round(float(sub["High"].max()), 2)}


def detect_divergence(price, indicator, lookback=60):
    """偵測頂/底背離"""
    ...
    if len(p) < 20 or len(ind) < 20:
        return result
    mid = len(p) // 2
    p1h, p2h = p.iloc[:mid].max(), p.iloc[mid:].max()
    i1h, i2h = ind.iloc[:mid].max(), ind.iloc[mid:].max()
    p1l, p2l = p.iloc[:mid].min(), p.iloc[mid:].min()
    i1l, i2l = ind.iloc[:mid].min(), ind.iloc[mid:].min()
    if p2h > p1h and i2h < i1h:
        result["bearish_divergence"] = True
    if p2l < p1l and i2l > i1l:
        result["bullish_divergence"] = True
    return result


def calculate_volume_quality(df, period=20):
    """量分位、漲跌量比、OBV 斜率/分位"""
    vol = df["Volume"]
    ...
    return {
        ...
        "obv_slope_20d": slope_n(obv, 20),
        "obv_percentile_252d": percentile_rank(obv, 252),
    }


def calculate_fibonacci_summary(df, lookback=120):
    """斐波那契摘要：只回傳離現價最近的支撐/壓力"""
    sub = df.tail(lookback)
    ...
    return {
        ...
        "fib_nearest_support_1": supports[0] if supports else None,
        "fib_nearest_support_2": supports[1] if len(supports) > 1 else None,
        "fib_nearest_resistance_1": resistances[0] if resistances else None,
        "fib_nearest_resistance_2": resistances[1] if len(resistances) > 1 else None,
    }


def detect_gaps_summary(df, lookback=30):
    """缺口摘要：只回傳最近 2 個未回補缺口"""
...
def detect_momentum_candle_patterns(df, n=5):
    """偵測動能延續型態：連續紅/黑K、跳空突破"""
    if df is None or len(df) < n:
        return {"consecutive_bull_bars": 0, "consecutive_bear_bars": 0,
                "gap_up_breakout": False, "gap_down_breakdown": False}
    tail = df.tail(n)
    cons_bull = cons_bear = 0
    for _, r in reversed(list(tail.iterrows())):
    ...
    if len(df) >= 2:
        last, prev2 = df.iloc[-1], df.iloc[-2]
        if float(last["Low"]) > float(prev2["High"]):
            gap_up = True
        if float(last["High"]) < float(prev2["Low"]):
            gap_down = True
    return {"consecutive_bull_bars": int(cons_bull), "consecutive_bear_bars": int(cons_bear),
            "gap_up_breakout": bool(gap_up), "gap_down_breakdown": bool(gap_down)}


def compute_institutional_features(chips_multi, price_now, price_20d_ago=None):
    """三大法人 JSON 特徵"""
    feat = {}
    if not chips_multi:
        feat["inst_data_available"] = False
        for k in ["foreign_5d_net", "foreign_20d_net", "trust_5d_net", "trust_20d_net",
                   "dealer_5d_net", "dealer_20d_net", "foreign_slope_20d", "trust_slope_20d",
                   "foreign_consecutive_days", "trust_consecutive_days",
                   "flag_foreign_divergence", "flag_inst_consensus_buy", "flag_inst_consensus_sell"]:
            feat[k] = None if "flag_" not in k else False
        return feat
    ...
    feat["foreign_5d_net"] = sum(f_vals[-5:]) if len(f_vals) >= 5 else sum(f_vals)
    feat["foreign_20d_net"] = sum(f_vals)
    feat["trust_5d_net"] = sum(t_vals[-5:]) if len(t_vals) >= 5 else sum(t_vals)
    feat["trust_20d_net"] = sum(t_vals)
    feat["dealer_5d_net"] = sum(d_vals[-5:]) if len(d_vals) >= 5 else sum(d_vals)
    ...
    for v in reversed(f_vals):
        if v > 0:
            ...
            break
    if cons == 0 and f_vals and f_vals[-1] < 0:
        for v in reversed(f_vals):
            if v < 0:
                cons -= 1
            else:
                break
    feat["foreign_consecutive_days"] = cons

    cons_t = 0
    for v in reversed(t_vals):
        if v > 0:
            cons_t += 1
        elif v < 0:
            cons_t -= 1
            ...
            break
    if cons_t == 0 and t_vals and t_vals[-1] < 0:
        for v in reversed(t_vals):
            if v < 0:
                cons_t -= 1
            else:
                break
    feat["trust_consecutive_days"] = cons_t

    # 旗標
    price_up_20d = price_now > price_20d_ago if price_20d_ago else False
...
def compute_decision_fields(c_now, atr_now, resistance, support, st_direction):
    """進出場決策欄位"""
    feat = {"decision_available": True}
    if atr_now and atr_now > 0:
        stop = round(c_now - 2 * atr_now, 2)
        feat["atr_stop_loss"] = stop
        feat["atr_stop_loss_pct"] = round((stop - c_now) / c_now * 100, 2)
    else:
        feat["atr_stop_loss"] = None
        feat["atr_stop_loss_pct"] = None
        feat["decision_available"] = False

    if resistance:
        feat["target_resistance"] = round(resistance, 2)
        feat["target_distance_pct"] = round((resistance - c_now) / c_now * 100, 2)
    else:
        feat["target_resistance"] = None
        feat["target_distance_pct"] = None

    stop = feat.get("atr_stop_loss")
    if stop and resistance and c_now != stop:
        feat["risk_reward_ratio"] = round((resistance - c_now) / (c_now - stop), 2)
    else:
        feat["risk_reward_ratio"] = None

    feat["nearest_support"] = round(support, 2) if support else None
    feat["support_distance_pct"] = round((support - c_now) / c_now * 100, 2) if support else None
    feat["flag_entry_trigger"] = bool(st_direction == 1) if st_direction is not None else False
    return feat
...
def get_margin_short_data(stock_id: str, trade_date, market_hint: str = None, max_back: int = 10):
    """修正版：上市/上櫃都嘗試（先嘗試 prefer 市場，失敗再試另一個）"""
    stock_no = _strip_suffix(stock_id)
    headers = {"User-Agent": "Mozilla/5.0", "Accept-Language": "zh-TW,zh;q=0.9"}
    ...
    def _try_twse(d):
        ...
        return {"id": f"{stock_no}.TW", "date": d.strftime("%Y-%m-%d"),
                ...
                "short_balance": s_bal, "short_change": s_chg, "error": None}, None
    ...
    def _try_tpex(d):
        ...
        try:
            ...
            if not code_col:
                return None, "TPEx 找不到代號欄"
            sub = df[df[code_col].astype(str).str.strip() == stock_no]
            if sub.empty:
                return None, f"TPEx 找不到 {stock_no}"
            row = sub.iloc[0]
            ...
            return None, f"TPEx: {e}"

    # 嘗試順序：優先市場 → 備用市場
    trials = [_try_tpex, _try_twse] if is_two else [_try_twse, _try_tpex]

    for back in range(max_back + 1):
        d = trade_date - timedelta(days=back)
        for try_fn in trials:
            result, err = try_fn(d)
...
def build_ai_features(stock_id: str, as_of_date=None) -> Dict[str, Any]:
    """
    主入口：回傳全數值化 JSON dict，供 AI / LLM 消費
    包含 120+ 欄位：價格、均線、動能、趨勢、量能、波動、支撐壓力、型態、籌碼、融資券、決策
    """
    ...
    stock_no = _strip_suffix(stock_id)
    feat: Dict[str, Any] = {"symbol": yf_ticker}
    ...
    if df_daily.empty and yf_ticker.endswith(".TW"):
        ...
        feat["symbol"] = yf_ticker
        df_daily = yf.download(yf_ticker, period="2y", interval="1d", progress=False, auto_adjust=False)
        df_daily = clean_yf_columns(_ensure_naive_index(df_daily))

    if df_daily.empty:
        return {"error": f"找不到 {stock_id}", "symbol": yf_ticker}

    chosen_ts = _nearest_trading_ts(df_daily, as_of_date)
    if chosen_ts is None:
        return {"error": "選定日期前無資料", "symbol": yf_ticker}

    df = df_daily.loc[:chosen_ts].copy()
    if df.empty or len(df) < 5:
        return {"error": "資料不足", "symbol": yf_ticker}

    latest_overall_ts = df_daily.index[-1]
    trade_date = chosen_ts.date()
    query_date = as_of_date if as_of_date else datetime.now().date()
    feat["price_date"] = str(trade_date)
    feat["query_date"] = str(query_date)

    # ── 基本價量 ──
    latest = df.iloc[-1]
    c_now = round(float(latest["Close"]), 2)
    feat["close"] = c_now
    feat["open"] = round(float(latest["Open"]), 2)
    feat["high"] = round(float(latest["High"]), 2)
    feat["low"] = round(float(latest["Low"]), 2)
    feat["volume"] = int(float(latest["Volume"])) if not pd.isna(latest.get("Volume", float("nan"))) else 0
    feat["volume_k"] = feat["volume"] // 1000
    ...
    ma5_now = round(float(ma5.iloc[-1]), 2) if not pd.isna(ma5.iloc[-1]) else None
    ma20_now = round(float(ma20.iloc[-1]), 2) if not pd.isna(ma20.iloc[-1]) else None
    ma60_now = round(float(ma60.iloc[-1]), 2) if not pd.isna(ma60.iloc[-1]) else None
    feat["ma5"] = ma5_now
    feat["ma20"] = ma20_now
    feat["ma60"] = ma60_now
    ...
    if ma20_now and ma20_now > 0:
        ma20_dev = round((c_now - ma20_now) / ma20_now * 100, 4)
        feat["ma20_dev_pct"] = ma20_dev
        dev20_series = (close - ma20) / ma20 * 100
        feat["ma20_dev_percentile_252d"] = percentile_rank(dev20_series, 252)
    else:
        feat["ma20_dev_pct"] = None
        feat["ma20_dev_percentile_252d"] = None

    if ma60_now and ma60_now > 0:
        ma60_dev = round((c_now - ma60_now) / ma60_now * 100, 4)
        feat["ma60_dev_pct"] = ma60_dev
        dev60_series = (close - ma60) / ma60 * 100
        feat["ma60_dev_percentile_252d"] = percentile_rank(dev60_series, 252)
    else:
    ...
    if ma5_now and ma20_now and ma60_now:
        ...
        ts = "insufficient_data"
    feat["trend_state"] = ts

    # 52 週位置
    high_52 = float(df.tail(252)["High"].max())
    low_52 = float(df.tail(252)["Low"].min())
    feat["pos_52w_pct"] = round((c_now - low_52) / (high_52 - low_52) * 100, 2) if high_52 != low_52 else 50
    ...
    try:
        ...
        if df_weekly is not None and not df_weekly.empty:
            wma20 = df_weekly["Close"].rolling(20).mean()
            feat["weekly_above_ma20"] = bool(float(df_weekly["Close"].iloc[-1]) > float(wma20.iloc[-1]))
        else:
    ...
    feat["rs_vs_bench_20d"] = rs_data["rs"] if rs_data else None
    feat["stock_ret_20d"] = rs_data["stock_ret"] if rs_data else None
    feat["bench_ret_20d"] = rs_data["bench_ret"] if rs_data else None

    # ── 量能 ──
    avg_vol_5 = float(df["Volume"].tail(5).mean())
    vol_now = float(latest["Volume"]) if not pd.isna(latest.get("Volume", float("nan"))) else 0
    feat["vol_ratio_5d"] = round(vol_now / avg_vol_5, 4) if avg_vol_5 > 0 else None

    prev = df.iloc[-2] if len(df) >= 2 else latest
    price_up = c_now > float(prev["Close"])
    vol_up = vol_now > avg_vol_5
    feat["flag_price_up_vol_up"] = bool(price_up and vol_up)
    feat["flag_price_up_vol_down"] = bool(price_up and not vol_up)
    feat["flag_price_down_vol_up"] = bool(not price_up and vol_up)

    vol_quality = calculate_volume_quality(df)
    feat.update(vol_quality)

    # ── 動能指標 ──
    rsi14 = calculate_rsi(close, 14)
    feat["rsi14"] = round(float(rsi14.iloc[-1]), 2)
    ...
    # KD 交叉
    k_f, d_f = k_val.dropna(), d_val.dropna()
    feat["flag_kd_golden_cross"] = bool(len(k_f) >= 2 and len(d_f) >= 2
                                        and k_f.iloc[-2] < d_f.iloc[-2] and k_f.iloc[-1] > d_f.iloc[-1])
    feat["flag_kd_death_cross"] = bool(len(k_f) >= 2 and len(d_f) >= 2
                                       and k_f.iloc[-2] > d_f.iloc[-2] and k_f.iloc[-1] < d_f.iloc[-1])
    ...
    dif_f, dea_f = dif.dropna(), dea.dropna()
    feat["flag_macd_golden_cross"] = bool(len(dif_f) >= 2 and len(dea_f) >= 2
                                          and dif_f.iloc[-2] < dea_f.iloc[-2] and dif_f.iloc[-1] > dea_f.iloc[-1])
    feat["flag_macd_death_cross"] = bool(len(dif_f) >= 2 and len(dea_f) >= 2
                                         and dif_f.iloc[-2] > dea_f.iloc[-2] and dif_f.iloc[-1] < dea_f.iloc[-1])
    ...
    atr_now = round(float(atr.iloc[-1]), 2) if not pd.isna(atr.iloc[-1]) else None
    feat["atr14"] = atr_now
    feat["atr14_pct"] = round(atr_now / c_now * 100, 4) if atr_now and c_now > 0 else None
    feat["atr14_percentile_252d"] = percentile_rank(atr, 252)

    ret = close.pct_change()
    feat["volatility_20d"] = round(float(ret.tail(20).std() * 100), 3) if len(ret) >= 20 else None
    feat["max_drawdown_20d"] = max_drawdown(close, 20)
    feat["max_drawdown_60d"] = max_drawdown(close, 60)

    # ── 布林 ──
    bbw, bb_upper, bb_lower = calculate_bbw(df)
    bbw_now = float(bbw.iloc[-1])
    bb_upper_now = float(bb_upper.iloc[-1])
    bb_lower_now = float(bb_lower.iloc[-1])
    feat["bb_width_pct"] = round(bbw_now, 4)
    bb_pos = (c_now - bb_lower_now) / (bb_upper_now - bb_lower_now) * 100 if bb_upper_now != bb_lower_now else 50
    feat["bb_position_pct"] = round(bb_pos, 2)
    feat["bb_width_percentile_252d"] = percentile_rank(bbw, 252)
    feat["flag_bb_squeeze"] = bool(percentile_rank(bbw, 252) is not None
                                   and percentile_rank(bbw, 252) < 0.10)
    ...
    feat["mfi14"] = round(float(mfi_series.iloc[-1]), 2)
    ...
    if avwap is not None and not avwap.empty and np.isfinite(float(avwap.iloc[-1])):
        ...
        feat["avwap_dev_pct"] = round((c_now - avwap_now) / avwap_now * 100, 3)
    else:
        feat["avwap_ytd"] = None
    ...
    # ── 斐波那契摘要 ──
    fib_sum = calculate_fibonacci_summary(df)
    feat.update(fib_sum)

    # ── 缺口 ──
    uf_gaps = detect_gaps_summary(df)
    feat["unfilled_gaps_count"] = len(uf_gaps)
    if uf_gaps:
        feat["nearest_gap_type"] = uf_gaps[-1]["type"]
        feat["nearest_gap_lower"] = uf_gaps[-1]["lower"]
        feat["nearest_gap_upper"] = uf_gaps[-1]["upper"]
    else:
    ...
    # ── 背離偵測 ──
    rsi_div = detect_divergence(close, rsi14)
    macd_div = detect_divergence(close, osc)
    feat["flag_bearish_divergence_rsi"] = bool(rsi_div["bearish_divergence"])
    feat["flag_bullish_divergence_rsi"] = bool(rsi_div["bullish_divergence"])
    feat["flag_bearish_divergence_macd"] = bool(macd_div["bearish_divergence"])
    feat["flag_bullish_divergence_macd"] = bool(macd_div["bullish_divergence"])

    # ── 動能K棒型態 ──
    candle_patterns = detect_momentum_candle_patterns(df)
    feat.update(candle_patterns)

    # ── 圖形型態 (優先級過濾, 最多 2 個, confirmed 一定是 bool) ──
    raw_patterns = detect_patterns(df)
    raw_patterns.sort(key=lambda p: (not bool(p.get("confirmed", False))))
    seen_types = set()
    chart_patterns = []
    for p in raw_patterns:
        ptype = p.get("pattern", "")
        category = "bearish" if any(kw in ptype for kw in ("頭肩頂", "M頭", "上升楔形", "Top", "Rising")) else "bullish"
        if category not in seen_types:
            seen_types.add(category)
            chart_patterns.append(p)
        if len(chart_patterns) >= 2:
            break
    feat["chart_pattern_count"] = len(chart_patterns)
    if chart_patterns:
        feat["chart_pattern_1"] = chart_patterns[0].get("pattern")
        feat["chart_pattern_1_confirmed"] = bool(chart_patterns[0].get("confirmed", False))
    else:
        feat["chart_pattern_1"] = None
        feat["chart_pattern_1_confirmed"] = False
    if len(chart_patterns) >= 2:
        feat["chart_pattern_2"] = chart_patterns[1].get("pattern")
        feat["chart_pattern_2_confirmed"] = bool(chart_patterns[1].get("confirmed", False))
    else:
    ...
    try:
        ...
        price_20d_ago = float(df["Close"].iloc[-20]) if len(df) >= 20 else None
    ...
    try:
        ...
        if isinstance(margin, dict) and margin.get("error") is None:
            feat["margin_balance"] = _safe_int(margin.get("margin_balance"))
            feat["margin_change"] = _safe_int(margin.get("margin_change"))
            feat["margin_usage_rate"] = margin.get("margin_usage_rate")
            feat["short_balance"] = _safe_int(margin.get("short_balance"))
            feat["short_change"] = _safe_int(margin.get("short_change"))
            m_bal = feat["margin_balance"]
            s_bal = feat["short_balance"]
            feat["short_margin_ratio"] = round(s_bal / m_bal * 100, 2) if (m_bal and s_bal and m_bal > 0) else None
            feat["margin_data_available"] = True
        else:
            for k in ["margin_balance", "margin_change", "margin_usage_rate",
...
def build_ai_features_json(stock_id: str, as_of_date=None) -> str:
    """回傳 JSON 字串"""
    feat = build_ai_features(stock_id, as_of_date)
    return json.dumps(feat, ensure_ascii=False, indent=2, default=str)
