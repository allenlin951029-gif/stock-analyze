...
# ---------------------------
def _parse_twse_csv_table(text: str):
...
def _parse_tpex_csv_table(text: str):
...
def get_institutional_data(stock_id, trade_date, market_hint=None):
    ...
    def try_twse(d_):
        ...
        df = _parse_twse_csv_table(r.text)
    ...
    def try_tpex(d_):
        ...
        df = _parse_tpex_csv_table(r.text)
...
# ---------------------------
# Margin / Short (融資融券) — 使用「可指定日期」的 CSV 來源
# ---------------------------
def _parse_twse_margn_csv(text: str):
    """
    TWSE MI_MARGN CSV：抓出包含「股票代號/股票名稱」的表格段落
    """
    text = text.replace("\r", "").replace("=", "")
    lines = [ln for ln in text.split("\n") if ln.strip()]
    start = None
    for i, ln in enumerate(lines):
        if ("股票代號" in ln) and ("股票名稱" in ln):
            start = i
            break
    if start is None:
        return None
...
def get_margin_short_data(stock_id: str, trade_date, market_hint=None):
    """
    回傳單位：張
    會往前找最多 10 天，回傳「實際有資料」的那一天
    {
      id, date,
      margin_bal, margin_chg,
      short_bal, short_chg,
      error
    }
    """
    ...
    prefer = _normalize_market_hint(market_hint)  # "TW"/"TWO"/None
    prefer_twse = True if prefer != "TWO" else False
    ...
    def pick_col(cols, wanted_norm_names: list[str]):
        mp = {_norm_col(c): c for c in cols}
        for wn in wanted_norm_names:
            if wn in mp:
                return mp[wn]
        return None
    ...
    for back in range(0, 10):
        ...
        for mkt_name, fn in markets:
            ...
            # 代號欄
            code_col = pick_col(cols, ["股票代號", "證券代號", "代號"])
            if not code_col:
            ...
            row = df[df[code_col].astype(str).str.strip() == stock_no]
            if row.empty:
                last_error = f"{mkt_name} 當日找不到 {stock_no}"
                continue

            # 欄位：盡量精準命中「前日/今日餘額」
            m_today_col = pick_col(cols, ["融資今日餘額", "融資餘額", "融資餘額(張)"])
            m_prev_col = pick_col(cols, ["融資前日餘額", "融資前日餘額(張)"])
            s_today_col = pick_col(cols, ["融券今日餘額", "融券餘額", "融券餘額(張)"])
            s_prev_col = pick_col(cols, ["融券前日餘額", "融券前日餘額(張)"])

            m_today = _safe_int(row.iloc[0][m_today_col]) if m_today_col else None
            m_prev = _safe_int(row.iloc[0][m_prev_col]) if m_prev_col else None
            s_today = _safe_int(row.iloc[0][s_today_col]) if s_today_col else None
            s_prev = _safe_int(row.iloc[0][s_prev_col]) if s_prev_col else None

            # 變動：優先用 today-prev
            m_chg = (m_today - m_prev) if (m_today is not None and m_prev is not None) else None
            s_chg = (s_today - s_prev) if (s_today is not None and s_prev is not None) else None
...
def analyze_stock_technical(stock_id: str) -> str:
    ...
    ms = get_margin_short_data(stock_id, trade_date=latest_trade_date, market_hint=yf_ticker)
    ...
    def fmt_delta(chg):
        ...
        if chg > 0:
            return f"增加 {chg:,} 張"
        if chg < 0:
            return f"減少 {abs(chg):,} 張"
        return "持平"

    if not isinstance(ms, dict) or ms.get("error") is not None:
        out.append(f"⚠️ 無法抓取融資融券（{ms.get('error') if isinstance(ms, dict) else '未知錯誤'}）")
    else:
        mb = ms.get("margin_bal")
        mc = ms.get("margin_chg")
        sb = ms.get("short_bal")
        sc = ms.get("short_chg")

        mb_txt = f"{mb:,} 張" if isinstance(mb, int) else "n/a"
        sb_txt = f"{sb:,} 張" if isinstance(sb, int) else "n/a"

        out.append(f"• 融資餘額: {mb_txt}（較前一交易日：{fmt_delta(mc)}）")
        out.append(f"• 融券餘額: {sb_txt}（較前一交易日：{fmt_delta(sc)}）")
        out.append(f"  (日期: {ms.get('date')}, 市場代碼推定: {ms.get('id')})")
    out.append("-" * 50)
