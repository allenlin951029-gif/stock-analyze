...
def clean_yf_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if isinstance(df.columns, pd.MultiIndex):
        # yfinance 有時會給 MultiIndex（例如 ('Close','2330.TW')）
        # 這裡盡量保留第一層或第二層讓欄位變成 Open/High/Low/Close/Volume
...
def get_institutional_data(stock_id, trade_date=None, market_hint=None):
    ...
    for back in range(0, 10):
        ...
        for mkt_name, fn in markets:
            ...
            dealer_col = _find_net_col(cols, "自營商") or next((c for c in cols if ("自營商" in str(c)) and ("買賣超" in str(c))), None)
            ...
            return {
                ...
                "dealer": dealer if dealer is not None else (row.iloc[0][dealer_col] if dealer_col else None),
                "error": None,
            }
...
def analyze_stock_technical(stock_id):
    ...
    vol = float(latest_daily["Volume"])
    vol_prev = float(prev_daily["Volume"])
    vol_in_lots = int(vol / 1000)
    vol_diff = int((vol - vol_prev) / 1000)
    vol_status = "量增" if vol > vol_prev else "量縮"
    print(f"• 成交量: {vol_in_lots} 張 ({vol_status}, 較昨 {vol_diff:+} 張)")
