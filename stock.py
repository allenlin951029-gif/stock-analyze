...
def calculate_volume_profile(df: pd.DataFrame, lookback: int = 60,
...
lookback: int = 60, pivot_n: int = 5) -> Dict[str, bool]:
“””
偵測頂/底背離
...
def compute_institutional_features(chips_multi: List[Dict],
price_now: float, price_20d_ago: float) -> Dict[str, Any]:
...
try:
    ...
    # TDCC 回傳格式: list of dicts with keys like
    # {'SCA_DATE': '20260213', 'HOLD_COM': '1-999', 'HOLD_NUM': '...', 'HOLD_UNIT': '...', 'HOLD_RATE': '...'}
    # 我們需要彙總：
    # 散戶 = 1-999 股 (約 1 張以下) 到 10,000 股 (10 張以下)
    # 大戶 = 400,001 股 (400 張以上) 到 more
    # whale = 1,000,001 股 (1000 張以上)
    ...
    for date_str in latest_dates:
        ...
        for row in day_data:
            ...
            # 判斷級距
            # 常見級距: 1-999, 1,000-5,000, 5,001-10,000, 10,001-15,000, ...
