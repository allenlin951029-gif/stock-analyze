...
def get_foreign_holding_ratio(stock_no: str) -> dict:
    ...
    url = f"https://www.twse.com.tw/rwd/zh/fund/MI_QFIIS?response=json&stockNo={stock_no}&queryType=1"
    try:
        r = requests.get(url, headers=headers, timeout=15)
        if r.status_code != 200:
            return {"ratio": None, "date": None, "error": f"HTTP {r.status_code}"}
...
def get_margin_short_data(stock_id: str, trade_date, market_hint: str = None, max_back: int = 10):
    ...
    for back in range(max_back + 1):
        ...
        try:
            ...
            if df is None or df.empty:
                last_error = f"TPEx 無資料"; continue
            colmap   = {_norm_col(c): c for c in df.columns}
            code_col = colmap.get("代號") or colmap.get("股票代號")
            if not code_col:
                last_error = "TPEx 找不到代號欄"; continue
            sub = df[df[code_col].astype(str).str.strip() == stock_no]
            if sub.empty:
                last_error = f"TPEx 找不到 {stock_no}"; continue
            row     = sub.iloc[0]
            ...
            last_error = f"TPEx 失敗: {e}"; continue
...
def analyze_stock_technical(stock_id: str, as_of_date=None) -> str:
    ...
    if   price_up and vol_up:     vol_price_label = "價漲量增（健康上漲，多方積極）"
    elif price_up and not vol_up: vol_price_label = "價漲量縮（動能不足，需觀察）"
    elif not price_up and vol_up: vol_price_label = "價跌量增（主力出貨警訊，偏空）"
    else:                         vol_price_label = "價跌量縮（正常回檔，賣壓有限）"
    ...
    if df_weekly_upto is not None and not df_weekly_upto.empty:
        ...
        out.append(f"  ▸ [週K] 收={wc:.2f}，20週均={wma20:.2f}，{'週線站上均線（多）' if wc > wma20 else '週線跌破均線（空）'}")
    ...
    if rsi6_pct is not None:
        out.append(f"    近一年百分位: 第 {rsi6_pct:.0f} 位（{'偏高' if rsi6_pct > 70 else ('偏低' if rsi6_pct < 30 else '正常')}）")
