...
"""
    ...
    with st.expander("🔥 提示詞 B：動能右側進攻名單（需開網頁搜尋）", expanded=False):
        prompt_b = r"""# 🔥 提示詞 B v3：動能右側進攻名單｜開網實戰最終版
...
- 不把「過熱」直接等於不能買。
- 要判斷它是「高動能續強」還是「末端陷阱」。
- late_trend / blowoff 盤允許小部位追強，但禁止無腦滿倉追高。
- 強股不一定會回 MA5，只等 MA5 可能會錯過主升段。
- RR 很差 + 背離，不可成為唯一 Tier1。
- Tier2-A 不是冷宮，T+1～T+3 續強可快速升 Tier2-B。
- 軟排除股可翻案，硬排除股不可翻案。
- 最新消息只做加減分，不可取代量價趨勢。
...
- 突破前高 → 可追，但必須有量能、消息催化或隔日不跌回突破點。
- 拉回不破 MA5 / AVWAP → 可接。
- 帶量站穩盤整突破點 → 可進。
- 強勢股不一定會給你便宜貨，等回 5% 有時就是放棄主升段。
- 右側交易不是看到漲就追，而是追「有續航力的強」。

## ❌ 絕對不做

- 不買跌深反彈的弱勢股。
- 不買跌破 MA20 / AVWAP / SuperTrend 的破線股。
- 不在下降趨勢摸底。
- 不為了便宜違反趨勢。
- 不用 Fib 38.2 / 50 / 61.8 當作逆勢買點。
- 不用 POC 當作跌深買點。
- 不在 ma20_dev_pct < -3% 時進場。
...
- Trump tariff Taiwan
- Fed latest rate cut
- Taiwan export orders latest
- Taiwan PMI latest
- Taiwan semiconductor news
...
```text
📡 市場補充：VIX{n}/{方向}｜美股{強/弱}｜費半{強/弱}｜美元/殖利率{風險}
🇺🇸 政策總經：Trump{Y/N}/{議題}｜Fed{偏鴿/中性/偏鷹}｜台灣總經{要點}
📰 個股消息：
  {代號}:{🟢需求/🟡成本/🔴轉嫁}/{最新要點}/{逆風有無}
三、第零步：填入市況

請先貼上 Prompt M 的 AI 區段。
...
我在這次分析中會做的調整:
├─ Tier1-A 正式進攻標準:{依激進度自填}
├─ Tier1-B 高動能小部位標準:{依市況自填}
├─ Tier2-B 續強試單標準:{依市況自填}
├─ 部位修正係數:×{依激進度自填}
├─ 進場優先序:{續強試單 / 拉回不破 / 突破追進 / 站穩關鍵位}
└─ 大盤約束:{1-2句具體說明}

範例：
「大盤週 RSI 接近 80，屬於 late_trend；不無腦追高，但若個股 RS 強、ADX 強、放量續強，允許 1%～2% Tier2-B 試單。」
五、候選股篩選

A 的角色已完成。
B 必須拿 sector JSON 原始資料重新判斷。
禁止參考 A 的分數。
...
模式 B：拉回不破 MA5 / AVWAP
模式 C：站穩關鍵位
5-2. Tier1-B 高動能小部位條件

Tier1-B 是「非常強，但偏熱，所以小部位進攻」。
...
□ 強勢股離 MA5 / AVWAP 太遠，拉回進場區距現價 >3%
□ trend_state = strong_uptrend，但 RSI / BB / MFI 偏熱，尚未出現續強確認
□ 量價強，但隔日是否承接未知
□ RR 偏低，但趨勢尚未破壞
□ 法人分歧，但價格結構仍強
□ 最新消息偏中性，缺明確催化
...
trend
late_trend
late_trend_or_blowoff
transition 但個股明顯強於大盤

且激進度在 50 以上時，不可把所有 RSI 高 / BB 高的股票直接排除。

若個股符合以下條件，可列為 Tier2-B 續強試單：
...
RSI >85：
高風險區，必須縮小部位。
若同時有長上影、爆量不漲、跌破前低，視為末端陷阱。
BB position 處理
BB position >100：
不直接排除。
若同時 RS 強、ADX 強、量能放大，標記為「噴出延伸候選」。

BB position >120：
高風險。
若同時爆量長上影、跌回突破點、法人轉賣，視為末端陷阱。
末端陷阱條件
...
RSI / MACD 背離只代表風險升高，不代表直接排除。

若背離同時出現以下任 2 條，才視為末端陷阱：
...
若只有背離，但價格仍收高、RS 強、ADX 強、量能健康，
則不得直接排除，只能降低部位。

八、排除區分類：硬排除 / 軟排除
8-1. 硬排除：不可翻案

任一觸發 → 直接排除，不得列 Tier2-B：

□ trend_state = downtrend / strong_downtrend
□ supertrend_bullish = false 且 weekly_trend_state ≠ uptrend
□ ma20_dev_pct < -3
□ 跌破 MA20 且未快速站回
□ 明確基本面利空
□ 法人同步大賣 + 價跌量增
...
若軟排除股在 T+1～T+3 放量突破並收高，
可解除排除，升為 Tier2-B 續強試單。

九、RS 弱勢排除修正版

原本 rs_vs_bench_20d < -3 直接排除太死板。

新規則：

若 rs_vs_bench_20d < -3，
但同時符合以下條件，可列 Tier2-A，不直接排除：
...
創 52W 高、突破前高、或上方無明確壓力時，不可只用舊壓力計算 RR。

原本：

target_resistance 太近 → RR 太低 → 排除
...
每檔 Tier1-A / Tier1-B / Tier2-B 必須指定模式。
...
有最新消息催化
進場：突破點 ～ 突破點 +1.5%
條件：成交量 ≥ 5日均量 1.2 倍，或 vol_ratio_5d > 1.2
停損：跌破突破點 或 -1ATR
模式 B：拉回不破 MA5 / AVWAP
...
late_trend / blowoff / transition 中的個股強勢
個股強勢過熱但未轉弱
不回 MA5，直接續強
進場：
1. 隔日突破前高且收盤不跌回突破點
或
2. 隔日不跌破前一日低點，且收盤仍在 MA5 上方
或
3. 收盤續創高，量能不低於 5日均量 80%
...
若 Tier2-A 個股在 T+1～T+3 內符合以下任 3 條，
可升為 Tier2-B 續強試單：
...
十五、Tier 標準：依激進度動態調整
激進度	Tier1-A 條件	Tier1-B 條件	Tier2-B 條件	偏好模式
≥80 全力	全7條 + 任一進場模式	可積極小部位	可積極試單	A / D
65-79 積極	全7條 + 風報合格	RS強 + ADX強 + 續強	可試單	A / B / D
50-64 中性	全7條 + ATR<5% + RR合理	小部位高動能	僅允許0.5%～1.5%試單	B / D
35-49 防禦	全7條 + RSI<70 + 低風險進場	原則不開	原則不開，除非極強	C
20-34 強防	暫停新進場	不開	不開	僅守持股
<20 危機	全停	不開	不開	守 ETF
中性盤特別規則
激進度 50～64 時：
- 不可無腦追高
- 但不可把 RSI 75～85、BB>100 一律排除
- 若符合 Tier1-B / Tier2-B 高動能續強條件，允許小部位試單
- 若 RR < 0.8 + 背離，不得列 Tier1-A
十六、分段資金配置
...
Tier1-A 正式進攻：3%～5%
Tier1-B 高動能小部位：2%～3%
Tier2-B 續強試單：0.5%～1.5%
Tier2-A 觀察：0%
軟排除觀察：0%
T+1～T+3 確認後
若 Tier2-A 觸發快速升級 → 加到 0.5%～1.5%
若 Tier2-B 續強成功 → 可加碼到 2%
若 Tier1-B 續強成功 → 可加碼，但單檔不超過 4%
若跌回突破點 / 跌破前低 → 取消或停損
單日新增曝險上限
transition：5%～8%
trend：8%～12%
late_trend：8%～12%，但單檔不得重倉
blowoff：5%～8%，只做試單
defensive：0%～5%
crisis：0%
十七、部位計算
...
個股修正
pos_52w_pct > 95：×0.85
RSI 75～85：不扣分，標記高動能區
RSI >85：×0.7
ATR% >5：×0.85
處置股：×0.5
flag_smart_short_signal：×0.85
RR <0.8：不得唯一 Tier1-A
RR <0.8 + 背離：降 Tier2-A / Tier2-B 或軟排除
爆量長上影：×0.5 或排除
最新消息明確利空：排除或降階
十八、輸出格式
18-1. 搜尋彙整
...
【{代號}】Tier1-A {🟢需求/🟡成本/🔴轉嫁} | 模式{A/B/C}
進場 {price範圍}（{突破X帶量 / 拉回MA5不破 / 站穩X+1%}）
停損 {stop}（{跌破突破點 / MA20 / 盤整下緣}）| ATR {n}%
催化：{具體要點；標明日期/來源}
🤖 我為什麼選它：{≤25字，動能右側視角}
部位：{n}%（基礎{n}% × 激進度×{n} × 個股×{n}）
18-3. Tier1-B 高動能小部位

每檔最多 5 行。

【{代號}】Tier1-B | 模式D 高動能小部位
進場：{突破X不跌回 / 不破前低續強 / 收盤創高}
停損：{跌回突破點 / 前低 / -1ATR} | 部位：{n}%
🤖 為何可攻：RS強 + ADX強 + 趨勢強，但過熱所以小部位
風險：{RSI高 / BB高 / 當沖熱 / 背離 / ATR高}
18-4. Tier2-B 續強試單

每檔最多 4 行。

【{代號}】Tier2-B | 模式D 續強試單
觸發：突破 {price} 後不跌回 / 隔日不破前低 / 收盤續創高
停損：{前低 / 突破點 / -1ATR} | 部位：0.5%～1.5%
🤖 為何可試：{RS強 + ADX強 + 趨勢強，但尚需確認}
18-5. Tier2-A 觀察

每檔最多 3 行。

【{代號}】Tier2-A 觀察
觸發條件：{拉回MA5不破 / 站回X帶量 / 突破Y後站穩}
🤖 為何只觀察：{≤20字}
18-6. 軟排除觀察

每檔一行。

{代號}：軟排除觀察，原因{RSI高 / BB高 / RR低 / 背離 / 當沖熱}；若T+1～T+3放量突破可翻案
18-7. 硬排除區

每檔一行。

{代號}：硬排除，原因{破MA20 / 跌破ST / RS弱且無轉強 / 法人倒貨 / 利空衝擊 / 弱勢反彈不追}
十九、資金配置輸出
現金 {套用M建議}%
Tier1-A 正式進攻 {n}%
Tier1-B 高動能小部位 {n}%
...
2. 進場條件是站穩，還是跌深？
3. 停損是趨勢破壞，還是亂設保底？
4. RSI / BB 高，是噴出延伸，還是末端陷阱？
5. RR <0.8 的股票，有沒有被錯放 Tier1-A？
6. 創高股是否錯用舊壓力當天花板？
7. 大盤激進度低時，部位是否有縮小？
8. 最新消息是否支持或至少沒有否定量價？
9. Tier2-A 是否有明確升級條件？
10. 軟排除股是否保留翻案機制？
二十一、硬規則
trend_state = strong_downtrend → 強制硬排除。
supertrend_bullish = false 且 weekly_trend_state ≠ uptrend → 硬排除。
ma20_dev_pct < -3 → 不進場。
利空消息直接衝擊基本面 → 受影響股排除。
全不合格 → 輸出「建議觀望」。
川普口頭威脅 ≠ 基本面崩壞；行政命令或正式政策衝擊才需降階。
消息必標來源與時間。
每檔 Tier1 ≤ 6 行；Tier2 ≤ 4 行；不寫冗長總結。
禁止逆勢逢低。
禁止參考 A 的分數。
進場區若距現價 >3%，原則降 Tier2-A；但若符合 Tier1-B / Tier2-B 續強條件，可用模式D小部位試單。
RR <0.8 + 背離，不得列為唯一 Tier1-A。
RSI 75～85 不可直接排除，需判斷是否為高動能續強。
BB position >100 不可直接排除，需判斷是否為噴出延伸。
創高股不可只用舊壓力計算 RR。
Tier2-A 不是永久觀察，T+1～T+3 續強可升級。
軟排除可翻案，硬排除不可翻案。
模式D 試單停損必須短，不得用過遠支撐硬扛。
"""
        # 動態組合：若有 M 結果，自動插入到 prompt 開頭
        m_result = st.session_state.get("prompt_m_ai_result", "").strip()
        if m_result:
