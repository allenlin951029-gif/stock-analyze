...
with st.sidebar:
    ...
    ai_prompt = """我上傳了最新的台股快篩數據，我的交易風格是「專注於高盈虧比、跟隨法人籌碼的短波段交易」。用我的標準分析這筆數據：趨勢第一、賠率第二、法人籌碼只做加減分，請幫我執行嚴格的汰弱留強。並且只要分析我上傳的檔案。
...
5. 不需要給買賣建議，只列出候選清單和每檔的關鍵數據摘要

【排版要求】
語氣要專業、果斷、像操盤手對老闆的精準匯報。請多用列點，並將股票代號、停損價位、盈虧比等關鍵字加粗。"""
    ai_prompt2="""你現在是一位嚴格、實戰導向的股票交易分析助手。請依照以下「逆勢買進 / 半信半疑買進 / 趨勢回檔買進」三套框架，分析我提供的大盤、個股、產業、消息面與持股資料，並輸出成一套可執行的交易決策。

【核心原則】
你不是要幫我猜最低點，也不是看到跌就說可以買。你要做的是判斷：
1. 這次下跌是「情緒失衡」還是「基本面惡化」
2. 這檔股票是「被錯殺」還是「真的轉弱」
3. 現在屬於哪一種買點：
   - A. 恐慌超跌買點
   - B. 半信半疑買點
   - C. 趨勢回檔買點
4. 如果都不符合，請直接明說「現在不是買點」

【判斷標準】

一、恐慌超跌買點（A 模式）
必須同時重點檢查以下條件：
- 下跌主因是否為外部衝擊，而非公司/產業基本面爆雷
  例如：地緣政治、利率預期、系統性風險、CTA/量化踩踏
- 大盤是否出現明顯恐慌：
  - 單日大跌 2.5% 以上
  - 或連兩日重挫
  - 或跌破前低後出現加速殺盤
...
【嚴格排除條件】
若出現以下任一情況，請直接列為「不適用這套買法」：
- 公司財報爆雷
- 指引下修
- 產業報價反轉
- 需求明顯轉弱
- 庫存惡化
- 法人降評理由屬基本面惡化，而不是短線情緒
- 股價跌破大結構後無法站回
- 主線龍頭全面翻空
...
Step 2：消息面與下跌原因拆解
- 這次下跌是外生衝擊還是內生惡化
- 哪些消息是短期情緒，哪些是會改變中期邏輯的核心變數

Step 3：主線與個股定位
- 判斷該股是否屬主線
- 判斷該股是龍頭、次龍頭、跟風股，還是純情緒股
- 若不是主線，直接降低評級
...
- 不要講廢話
- 不要只說「視情況而定」
- 若資料不足，請直接講缺什麼資料，不要硬編
- 若結論是不該買，請直接說，不要安慰我"""
    st.code(ai_prompt, ai_prompt2,language="text")
