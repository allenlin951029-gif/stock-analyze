...
with st.sidebar:
    ...
    with st.expander("🌐 提示詞 M：市況判讀（先跑這個）", expanded=False):
        prompt_m = r"""# 🌐 提示詞 M v2:市況判讀 + 題材熱度地圖

> **v2 變更摘要**(對應新版 market_features JSON):
> - 步驟 1 多讀 11 個新欄位
> - 步驟 4-B 過熱分 → 對齊系統 `market_overheat_score`(7:3 合併池)
> - 步驟 4-C 系統風險 → 改用 `flag_vix_spike` + `flag_foreign_futures_extreme_short` + TWII 跌幅階梯
> - 步驟 4-A 趨勢面加 `ma20_dev_pct` 抑制
> - AI 區段新增 3 行:資料新鮮度、籌碼/散戶結構、系統旗標
...
**🆕 v2 新增欄位**(以下都是 JSON 已內建,直接取用):
- `actual_data_date`, `data_lag_days` ← 資料新鮮度
- `market_overheat_score` (0-100), `market_overheat_label`, `market_overheat_breakdown`
- `market_ma20_dev_pct` ← 大盤 MA20 偏離率
- `market_foreign_futures_net_oi`, `_long_oi`, `_short_oi`, `_position_state`, `_5d_change`
- `market_margin_balance`, `market_margin_5d_change_pct`, `market_short_balance`, `market_margin_short_ratio`
- `flag_vix_spike`, `flag_foreign_futures_extreme_short`, `flag_margin_overheat`
- `market_flags` 陣列(已整合上方所有觸發旗標)
- `vix_change_pct`
...
#### A. 趨勢面分(市場結構是否健康)
- TWII strong_uptrend + 週 uptrend + ST bullish + `ma20_dev_pct < 7%` → 90+
- 🆕 趨勢盤但 `weekly_rsi ≥ 80` **或** `ma20_dev_pct ≥ 7%` → 70-80(過度延伸警示)
- 🆕 趨勢盤但 `weekly_rsi ≥ 85` **且** `ma20_dev_pct ≥ 7%` → 60-70(吹哨頂訊號)
- 震盪盤 / 轉折期 → 40-60
- 跌破週 MA20 → 20-40
- weekly downtrend → 0-20

#### B. 過熱程度分(越高越過熱)— 🆕 v2 直接對齊系統

```
B = market_overheat_score × 0.7 + (合併池過熱比例 × 100) × 0.3
```

**為什麼這樣設計**:
- `market_overheat_score` 已加權 7 個訊號(weekly_rsi 30 + daily_rsi 15 + ma20_dev 15 + bb_pos 10 + vix_spike 10 + foreign_futures 15 + margin 5),涵蓋大盤層面
- 合併池過熱比例反映個股層面熱度
- 7:3 偏重系統面,避免個股篩選結果的池子組成偏誤

**對照表**(以 `market_overheat_label` 為輔):

| market_overheat_score | label | B 大致範圍(若合併池過熱 ~50%)|
|---|---|---|
| ≥ 80 | extreme_overheat | 71-100 |
| 65-79 | overheat | 61-70 |
| 50-64 | warm | 50-60 |
| 30-49 | neutral | 30-49 |
| < 30 | cool | 0-29 |
...
2. 🆕 flag_vix_spike == true        → +20

3. 🆕 flag_foreign_futures_extreme_short == true   → +20

4. TWII 單日跌幅(取最嚴重一條):
   twii_change_pct < -1%             → +10
   twii_change_pct < -2%             → +20(取代 +10)
   twii_change_pct < -3%             → +35(取代 +20)

5. flag_margin_overheat == true     → +10(融資 5 日急增,散戶接刀)

6. 突發政策事件(用戶判斷,如川普行政命令、地緣衝突)→ +20
```
...
📋 資料新鮮度:actual={actual_data_date} / 落後{data_lag_days}天 {若>2加 ⚠️}

🌐 市場狀態:{trend_state日}(系統建議:{suggested_regime})

📊 大盤客觀:
   TWII {n} ({±%}%) / 日RSI {n} / 週RSI {n} / VIX {n}{level}({±%}%{若flag_vix_spike+🚨})
   趨勢 {trend_state}日/{weekly}週 / ST{狀態} / BB{n}% / ADX{n} / MA20偏離 {±n}%

💰 籌碼/散戶結構:
   外資期淨單 {±n}口({position_state}{若extreme_short+🚨})/ 5日{±n}
   整體融資餘額 {n}萬({5d_change_pct ±%}%{若flag_margin_overheat+🚨})
   {margin_data_available=false 時:此區段標「資料未到位(FinMind T+1)」}
...
🌡️ 激進度量表:{n}/100 → {🟢全力/🟢積極/🟡中性/🟠防禦/🔴強防/🚨危機}
   ├─ 趨勢面:{n}/100({評語})
   ├─ 過熱程度:{n}/100({評語},系統過熱分對照 {market_overheat_score}/100「{label}」)
   ├─ 系統風險:{n}/100({評語},觸發旗標 {market_flags 列出})
   └─ 廣度健康:{n}/100({評語})
   💰 建議現金比:{n}%

🚩 系統旗標(JSON 原樣):{market_flags 陣列}
...
7. 不評論個股、不給投資建議
8. 🆕 **`data_lag_days > 2` 時**,AI 區段「📋 資料新鮮度」加上 ⚠️ 並在「⚠️ 警示」第一條註明「資料延遲 N 天,訊號可能滯後」
9. 🆕 **`margin_data_available == false` 時**,「💰 籌碼/散戶結構」標「資料未到位」,系統風險分 C 的步驟 5 跳過(不加 flag_margin_overheat 分)
10. 🆕 **`market_overheat_score` 與用戶區段池子熱度衝突時**(如系統 score 80 但池子全冷清),AI 區段「🎯 推斷」需指出「大盤過熱但個股廣度退潮 → 權值噴發但內部退潮」
...
"""
    ...
    with st.expander("🔥 提示詞 B v3.1：動能右側進攻名單（需開網頁搜尋）", expanded=False):
        prompt_b = r"""# 🔥 提示詞 B v3.2：動能右側進攻名單｜彈性指標進階版
...
⭐ **v3.2 核心改動**：
1. 加強布林通道訊號（squeeze / fire / width / above_upper）
2. 動能評分加入「警訊扣分」與「加分」機制
3. **指標使用不再固定 5 個**：AI 應視標的情境主動納入 JSON 中任何有意義的欄位作為輔證、扣分、加分，最大化資訊利用
4. 移除「系統內建進場觸發 flag」相關判斷（不依賴此欄位）

每檔輸出精簡：
- Tier1-A：每檔 ≤ 8 行（含動能評分 + 監控訊號 + 補充指標）
- Tier1-B：每檔 ≤ 7 行
- Tier2-B：每檔 ≤ 6 行
- Tier2-A：每檔 ≤ 3 行
- 軟排除/硬排除：一行帶過
...
- 拉回不破 MA5 / AVWAP → 可接
- 帶量站穩盤整突破點 → 可進
- **bb_squeeze 後 squeeze_fire 觸發 → 待噴強訊號**
...
□ 無 RSI/MACD 明顯背離
□ 最新消息無明確利空
□ 布林通道未過熱（bb_position_pct < 100 且非 above_bb_upper_in_high_vol）
□ 動能評分 v3.2（基礎 + 警訊調整後）≥ 80
```
...
- 法人分歧，但價格結構強
- 最新消息中性，缺催化
- **bb_squeeze 中尚未 squeeze_fire 觸發**
...
- 當沖偏熱
- 位置過高
- **bb_width_pct 異常窄（< 20 百分位）但無 squeeze_fire**
...
| 60-90 | 健康強勢區 |
| 90-100 | 接近上軌，偏熱但仍可進 |
| 100-120 | 不直接排除；若 RS 強+ADX 強+量增 → 「噴出延伸候選」 |
| >120 | 高風險；同時爆量長上影/跌回突破點/法人轉賣 → 末端陷阱 |

### bb_width_pct + bb_width_percentile_252d（v3.2 新增）
- `bb_width_percentile_252d < 0.2`：通道極窄（壓縮中）→ 配合 flag_bb_squeeze 觀察待噴
- `bb_width_percentile_252d > 0.8`：通道極寬 → 已在主噴段，追價風險升高
- 通道急速擴張（width 1 週內翻倍）→ 已噴出，不宜追

### flag_bb_squeeze / flag_bb_squeeze_fire（v3.2 新增）
- `flag_bb_squeeze = true` 且 `flag_bb_squeeze_fire = false`：通道收斂中，等爆發 → **Tier2-A 觀察**
- `flag_bb_squeeze_fire = true`：剛突破壓縮 → **進場模式 C 強訊號，可升 Tier1-B/Tier2-B**
- 持續 squeeze 但無 fire → 動能不足

### flag_above_bb_upper（v3.2 新增）
- 收盤站上 BB upper（特別是 entry_warnings 含 above_bb_upper_in_high_vol）：
  - 若 RS 強 + ADX > 40 + 量增 → 「噴出延伸候選」可進但部位 ×0.7
  - 若無上述支撐 → 末端陷阱候選，降階
...
## 模式 C：盤整突破（v3.2 強化布林訊號）
適用：盤整收斂、`flag_bb_squeeze = true` 待噴
- 進場：盤整上緣突破點 / `flag_bb_squeeze_fire = true` 當下
- 條件：量 ≥ 1.5×五日均量
- 停損：盤整下緣 -0.5ATR / 跌破 BB middle line
...
- `rs_vs_bench_63d > +10` 或 `rs_improving = true`（中期 RS 仍強）
- RS 5D/10D 明顯轉強
- 放量突破
- 族群同步轉強
- weekly = uptrend
...
## 8-4. 短期 RS 弱但中期 RS 強的補漲股（v3.2 新增）

當 `rs_vs_bench_20d < +5` 但 `rs_vs_bench_63d > +20`：
- 代表近期被洗，但中期動能未失
- 若同時 OBV 創高（`obv_percentile_252d > 0.9`）+ 量能不弱
- → 列 Tier2-A 觀察 + 等突破日線壓力升 Tier2-B
- 避免錯過「主流補漲股」（如 v3.1 漏掉 6285 的案例）
...
| 50-64 中性 | 全條 + ATR<5%+RR合理 | 小部位高動能 | 0.5-1.5% 試單 | B / C / D |
...
- 爆量長上影：×0.5 或排除
- 最新消息明確利空：排除或降階
- **bb_position_pct > 120：×0.7（v3.2 新增）**
- **margin_usage_rate > 50%：×0.85（散戶槓桿過熱）**
...
## 12-2. Tier1-A 正式進攻（每檔 ≤ 8 行）

```
【{代號}】Tier1-A {🟢/🟡/🔴} | 模式{A/B/C/D}
進場 {price 範圍}（{進場條件}）
停損 {stop} | ATR {n}% | RR {n}
催化：{要點；標日期/來源}
🔥 動能評分：基礎 RS{n}/ADX{n}/RSI{n}/BB{n}/Vol{n} = {x}/100；調整後 {y}/100 {🚀爆發/💪續強/⚡偏熱/🌊普通}
   調整明細：{警訊扣分 / 加分項，視標的需要列舉}
🤖 我為什麼選它：{≤30字 動能右側視角，可引用任何輔證指標}
👁️ 監控訊號：跌破 {n}（前低）/ 量縮<0.7 / RSI 跌破 70 / BB 翻空 → 立即降階或停損
```

## 12-3. Tier1-B 高動能小部位（每檔 ≤ 7 行）
...
🔥 動能評分：基礎 {x} / 調整後 {y} {等級}
   調整明細：{視標的列舉 — 例如 obv 強 +5、mfi 過熱 -5、bb_squeeze_fire +10}
🤖 為何可攻：{≤25字，靈活引用 RS63d / OBV / MTF / squeeze 等任一輔證}
👁️ 監控訊號：跌破 {n} / 量縮<0.7 / RSI<70 → 立即降階
```

## 12-4. Tier2-B 續強試單（每檔 ≤ 6 行）

```
【{代號}】Tier2-B | 模式 {A/B/C/D}
觸發：{觸發 1/2/3 中選一}
停損：{前低 / 突破點 / -1ATR} | 部位：0.5-1.5%
🔥 動能評分：基礎 {x} / 調整後 {y} {等級}
   {若有關鍵警訊或加分，1 行內列舉}
👁️ 監控訊號：{跌破 X / 量縮 / RSI 反轉 / BB 訊號} → 取消試單
```
...
```
【{代號}】Tier2-A 觀察
觸發條件：{拉回 MA5 不破 / 站回 X 帶量 / 突破 Y 站穩 / squeeze_fire 觸發}
🤖 為何只觀察：{≤25字，可引用 OBV、RS63d、MTF、bb 等任一指標說明}
```

## 12-6. 軟排除（每檔一行）

```
{代號}：軟排除，原因 {RSI高 / BB高 / RR低 / 背離 / 當沖熱 / OBV背離 / MFI過熱 / 融資過熱}；若 T+1~T+3 放量突破可翻案
```

## 12-7. 硬排除（每檔一行）

```
{代號}：硬排除，原因 {破 MA20 / 跌破 ST / RS弱無轉強 / 法人倒貨 / 利空衝擊 / 處置股}
```
...
- 軟排除可翻案，硬排除不可翻案
- **OBV 雙週期背離 + RS 未改善 + MFI 過熱 → 至少降階一級（v3.2 新增）**
...
4. 每檔行數遵守
5. 不寫冗長總結
6. **指標使用不固定 — AI 視標的情境主動納入 JSON 中有意義的欄位（v3.2 新原則）**
...
5. 大盤激進度低 → 部位有縮小嗎？
6. **OBV / MTF / RS_improving 是否與價格訊號一致？背離則需降階（v3.2 新增）**
...
```
🔥 動能評分（基礎）：RS{n}/ADX{n}/RSI{n}/BB{n}/Vol{n} = {x}/100
```
...
## 警訊扣分（v3.2 新增；AI 視標的情境靈活套用，不需逐項勾選）

下列為「常用扣分情境」，AI 應視標的實際資料判斷是否觸發。**不限於這份清單** — JSON 中若有其他明顯轉弱訊號（如 mtf 失效、ma 斜率反轉、巨幅借券增加等），都可酌量扣分。

| 條件 | 扣分 | 說明 |
|---|---|---|
| `mtf_alignment` 非 aligned_bull | -10 | 多時間框架失效 |
| `obv_slope_5d < 0` 且 RS20 > 10 | -10 | 量價短期背離 |
| `obv_slope_5d 與 obv_slope_20d 同時 < 0` | -15 | OBV 雙週期背離（更嚴重）|
| `rs_improving = False` | -5 | RS 不再改善 |
| `margin_usage_rate > 30` | -5 | 散戶槓桿過熱 |
| `macd_osc_slope_5d < -10` | -10 | MACD 動能急速反轉 |
| `rsi14_slope_5d < -2` 且 RSI > 75 | -5 | 過熱區 RSI 反轉 |
| `kd_k < kd_d` 且 kd_d > 80 | -5 | 高檔 KD 死叉 |
| `mfi14 > 90` | -5 | 資金流量極端過熱 |
| `bb_width_percentile_252d > 0.85` 且 RSI > 80 | -5 | BB 通道極寬+RSI極熱 → 噴出末段 |
| `lending_volume_5d_change > +5000` 且 RSI > 75 | -5 | 借券大增+過熱 |
| 其他 AI 判斷有意義的轉弱訊號 | -5 ~ -10 | 自行判斷 |

## 加分機制（v3.2 新增；同樣靈活）

| 條件 | 加分 | 說明 |
|---|---|---|
| `up_down_vol_ratio_20d > 2` | +5 | 多頭量能優勢 |
| `obv_percentile_252d > 0.95` | +5 | OBV 創 252 日新高 |
| `flag_bb_squeeze_fire = True` | +10 | BB 壓縮後爆發 |
| `rs_vs_bench_63d > +20` 且 RS20 偏弱 | +5 | 中期 RS 強的補漲股 |
| `foreign_consecutive_days > 5` | +5 | 外資連續強買 |
| `revenue_momentum = accelerating` 且 EPS 加速 | +5 | 基本面加速確認 |
| 其他 AI 判斷有意義的強化訊號 | +5 ~ +10 | 自行判斷 |

## 輸出格式

```
🔥 動能評分：基礎 {x}/100；調整後 {y}/100 {等級}
   調整明細：{扣分項，列舉 1-3 個最重要的}/{加分項，列舉 1-2 個最重要的}
```

範例：
```
🔥 動能評分：基礎 80/100；調整後 70/100 💪續強
   調整明細：obv 短背離 -10、MFI 91 過熱 -5、多頭量勢 +5
```
...
# 十六、彈性指標原則（v3.2 核心新原則）

## 為什麼要彈性？

v3.1 的固定 5 維評分會錯過很多有價值的訊號。例如：
- 一檔股票 RS20 弱但 RS63d 極強 → 可能是補漲股（v3.1 會直接降階）
- 一檔股票 RSI/BB/ADX 全綠但 OBV 已雙週期背離 → 可能是末段（v3.1 看不出來）
- 一檔股票表面普通但 bb_squeeze_fire 剛觸發 → 重要進場訊號（v3.1 沒抓）

## AI 應如何運用？

1. **基礎評分必算**（RS/ADX/RSI/BB/Vol）— 維持框架一致性與可比較性
2. **警訊扣分視情境**：發現 JSON 中明顯的轉弱訊號（不限於清單列舉的），可主動扣分
3. **加分視情境**：發現 JSON 中明顯的強化訊號（不限於清單列舉的），可主動加分
4. **輔證指標靈活引用**：在「我為什麼選它」一行中，可引用任何有意義的指標作為解釋
5. **每檔的指標組合可以不同**：權值股與小型噴出股、消息股與題材股的關注焦點本來就不同

## 不限於常用指標

JSON 中還有許多欄位（如 `chart_pattern_1`、`flag_supertrend_flip_bull`、`mfi14`、`avwap_dev_pct`、`max_drawdown_60d`、`beta_60d` 等），AI 應視個股情境主動使用，**不要因為「清單沒列」就忽略**。
